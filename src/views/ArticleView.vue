...
<!--onMounted(async () => {-->
<!--  try {-->
<!--    const response = await axios.get('/api/article'); // 获取文章列表数据-->
...
<!--// 文章概述函数，截取前150个字符-->
<!--const getArticleExcerpt = (context) => {-->
...
<!--      &lt;!&ndash; 第一行 &ndash;&gt;-->
<!--      <div class="row f_row">-->
<!--        <div class="box" v-for="(article, index) in articles.slice(0, 3)" :key="'f_row-' + index">-->
<!--          <router-link class="pic" :to="{ path: '/article', query: { articleId: article.article_ID, articleTitle: article.title } }">-->
<!--            <img :src="require(`@/${article.Picture_URL}`)" alt="" />-->
<!--            <p class="article_title">{{ article.title }}</p>-->
<!--          </router-link>-->
<!--          <p class="article_excerpt">{{ getArticleExcerpt(article.context) }}</p>-->
<!--        </div>-->
<!--      </div>-->

<!--      &lt;!&ndash; 第二行 &ndash;&gt;-->
<!--      <div class="row s_row">-->
<!--        <div class="box" v-for="(article, index) in articles.slice(3, 6)" :key="'s_row-' + index">-->
<!--          <router-link class="pic" :to="{ path: '/article', query: { articleId: article.article_ID, articleTitle: article.title } }">-->
<!--            <img :src="require(`@/${article.Picture_URL}`)" alt="" />-->
<!--            <p class="article_title">{{ article.title }}</p>-->
<!--          </router-link>-->
...
<!--        <div class="box" v-for="(article, index) in articles.slice(6, 9)" :key="'th_row-' + index">-->
<!--          <router-link class="pic" :to="{ path: '/article', query: { articleId: article.article_ID, articleTitle: article.title } }">-->
<!--            <img :src="require(`@/${article.Picture_URL}`)" alt="" />-->
<!--            <p class="article_title">{{ article.title }}</p>-->
<!--          </router-link>-->
...
<!--.box p.article_excerpt {-->
<!--  flex: 0 0 20%; /* 固定概述占高度的 20% */-->
<!--  display: flex;-->
<!--  align-items: center;-->
